...
function createHeroCard(hero) {
    ...
    card.innerHTML = `
        ...
        <div class="rating-container" data-hero-id="${hero.id}">
...
function handleRating(e) {
    ...
    localStorage.setItem(`hero-${heroId}-rating`, rating);
}

function initializeRatings() {
    document.querySelectorAll('.rating-container').forEach(container => {
        const heroId = container.dataset.heroId;
        const savedRating = localStorage.getItem(`hero-${heroId}-rating`);
        
        if (savedRating) {
            container.querySelectorAll('.heart').forEach((heart, index) => {
                heart.classList.toggle('active', index < savedRating);
            });
        }
